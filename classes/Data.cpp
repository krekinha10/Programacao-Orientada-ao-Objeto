...
void Data::AddSubMeses( int Meses )
{
    ...
    if ( Meses >= 0 )               // PARA AVANÇAR MESES EM UMA DATA
    {
        for( int i = 0; i < Meses; i++ )    // ITERAMOS DE 0 A MESES - 1
        {
            if ( NovoMes == 12 )    // SE O MÊS ATUAL É O ÚLTIMO MÊS DO ANO
            {
                NovoAno++;          // INCREMENTA O ANO
                NovoMes = 1;        // ATUALIZA O MÊS PARA JANEIRO
            }
            else
            {
                NovoMes++;          // SENÃO, SÓ INCREMENTA O MÊS ATUAL
            }
        }
    }
    else if ( Meses < 0 )           // PARA DIMINUIR MESES A UMA DATA
    {
        for( int i = 0; i > Meses; i-- )    // ITERAMOS DE 0 ATÉ MESES + 1
        {
            if ( NovoMes == 1 )     // VERIFICAMOS SE O MÊS ATUAL É JANEIRO
            {
                NovoAno--;          // SE FOR, DECREMENTAMOS O ANO
                NovoMes = 12;       // ATUALIZAMOS O VALOR DO MÊS PARA DEZEMBRO
            }
            else
            {
                NovoMes--;          // SE NÃO, SÓ DECREMENTA O MÊS
            }
        }
    }

    setAno( NovoAno );              // ATUALIZA A DATA COM OS NOVOS
    setMes( NovoMes );              // VALORES DO ANO E DO MÊS
}
...
void Data::AddSubAnos( int Anos )
{
    ...
    if ( Anos >= 0 )                      // PARA AVANÇAR MESES EM UMA DATA
    {
        for( int i = 0; i < Anos; i++ )   // ITERAMOS DE 0 ATÉ ANOS - 1
            NovoAno++;                    // INCREMENTAMOS O ANO ATUAL
    }
    else if ( Anos < 0 )                  // PARA DIMINUIR ANOS EM UMA DATA
    {
        for( int i = 0; i > Anos; i-- )   // ITERAMOS DE 0 ATÉ ANOS + 1
            NovoAno--;                    // DECREMENTAMOS O ANO ATUAL
    }

    setAno( NovoAno );                    // ATUALIZA A DATA COM O NOVO VALOR PARA O ANO
}
